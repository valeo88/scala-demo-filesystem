...
import com.valeopopov.demofilesystem.files.{Directory, File}
  ...
  def doEcho(state: State, contents: String, filename: String, append: Boolean): State = {
    if (filename.contains(Directory.SEPARATOR))
      ...
      if (newRoot == state.root)
        ...
        State(newRoot, newRoot.findDescendant(state.wd.getAllDirsInPath).get)
    }
  }

  private def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String, append: Boolean): Directory = {
    if (path.isEmpty) currentDirectory
      ...
      else if (dirEntry.exists(_.isDirectory))
        currentDirectory
      else
        if (append) currentDirectory.replaceEntry(path.head, dirEntry.map(_.asFile).map(_.appendContents(contents)).get)
        else currentDirectory.replaceEntry(path.head, dirEntry.map(_.asFile).map(_.setContents(contents)).get)
    } else {
      val nextDirectory = currentDirectory.findEntry(path.head).map(_.asDirectory).get
